...
#
# Usage:
#   build_comment_object "1" "@loop fix" "a.go" "10" "RIGHT" "@@" "" "pull_request_review_comment" "me"
#
#######################################
function build_comment_object {
    ...
    local source="${8:-}"
    local actor="${9:-}"
    local comment_id_json="" line_json="" in_reply_to_json=""
...
function filter_api_comments_to_objects {
    ...
    local mention="${PR_MENTION:-@loop}"
    local -a objects=()
    local item body assoc actor_type comment_id path line side diff_hunk in_reply_to actor eyes obj
...
function hydrate_pr_env_from_event {
    ...
    local comment_in_reply_to actor_type actor actor_association
