...
description: >-
  ...
  Triggers via on-loop-docs-triage.yaml cron; never user-invoked.
license: Apache-2.0
metadata:
  author: y-miyazaki
  version: "2.4.0"
...
At `L2`/`L3`, edit High-Priority items within [category-scope.md](references/category-scope.md).
...
1. Parse [category-input-schema.md](references/category-input-schema.md). If `skip` or no actionable `findings`, emit report with Summary `No documentation impact detected`; stop.
2. Classify per [common-checklist.md](references/common-checklist.md); fix High-Priority items from `findings`.
3. At `L2`/`L3`, edit files within [category-scope.md](references/category-scope.md) allowlist.
