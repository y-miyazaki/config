...
- [Agent Config Distribution: APM](#agent-config-distribution-apm)
  - [Guidelines](#guidelines-2)
- [Additional Candidate Category: Agent Capability Distribution Model](#additional-candidate-category-agent-capability-distribution-model)
...
| Agent Config Distribution    | APM                                 | Agent 設定資産 (MCP/Instructions/Skills/Hooks) 配布 |
| Capability Distribution Model | APM / Git 同期 / Agent-native Plugin | Skill/Plugin をどう配布・再利用するかを設計        |
...
## Agent Config Distribution: APM

| 比較項目                 | APM                                                     |
| ------------------------ | ------------------------------------------------------- |
| ツール種別               | Agent 設定配布マネージャー                              |
| 主用途                   | Agent 設定配布 (MCP/Instructions/Skills/Hooks)         |
| 必要となるケース         | 複数環境へ Agent 設定資産を再現配布したい場合           |
| ロック運用               | `apm.lock.yaml`                                         |
| 代表コマンド             | `apm install`, `apm install --frozen`, `apm audit --ci` |
| 配布対象                 | Agent 設定資産                                          |
| 再現性                   | 高 (`--frozen` 運用)                                     |

### Guidelines

**→ APM は Agent 設定配布の要件がある場合に採用する。言語依存の依存管理は各言語比較へ分離する。**

- Agent 設定資産を複数環境で再現配布する要件がある場合は APM を採用
- 言語依存パッケージ管理 (go mod / npm,pnpm / uv,pip) は言語別 comparison で評価する
- 単一ツールへ統一せず、責務境界 (設定配布/実行依存) で管理
- 併用時はロックファイル検証を CI で独立実行する

## Additional Candidate Category: Agent Capability Distribution Model

`APM` と同レイヤで比較すべきなのは「言語依存 PM」ではなく「Agent 能力の配布方式」。

| 比較項目                 | APM                                           | Git 同期モデル (submodule/symlink/template) | Agent-native Plugin モデル                      |
| ------------------------ | --------------------------------------------- | -------------------------------------------- | ----------------------------------------------- |
| 配布単位                 | 設定パッケージ                                | リポジトリ/ファイル                           | Plugin/拡張機能                                 |
| 代表ユースケース         | MCP/Instructions/Skills/Hooks の再現配布      | Skill/Rules の共有                            | Agent 機能追加の即時導入                         |
| バージョン固定           | 高 (`apm.lock.yaml`)                          | 中 (Git 運用次第)                             | 低〜中 (提供側更新に依存)                        |
| 監査/統制                | 高 (`apm audit`)                              | 中 (レビュー運用依存)                         | 中 (Marketplace/提供元ポリシー依存)              |
| 導入速度                 | 中                                             | 低〜中                                        | 高                                              |
| ベンダーロックイン       | 低〜中                                         | 低                                             | 中〜高                                          |
| 想定する例               | APM package                                   | GitHub Skills / Claude Skills の Git 共有運用 | Claude の plugin 的拡張、拡張マーケット導入モデル |

### Guidelines

**→ 比較対象は「npm/pip/go mod」ではなく、APM と同じ配布レイヤのモデルに限定する。**

- ガバナンスと再現性を優先するなら APM
- 小規模チームで運用コスト最小化を優先するなら Git 同期モデル
- 導入速度を最優先するなら Agent-native Plugin モデル
