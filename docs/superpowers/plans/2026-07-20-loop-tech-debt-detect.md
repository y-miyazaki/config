...
- `scripts/lib/` only via `bash scripts/ai/sync_skill_lib.sh` (never hand-edit skill lib copies).
...
- Run: `bash scripts/ai/sync_skill_lib.sh` (creates `scripts/lib/`)
...
bash scripts/ai/sync_skill_lib.sh
```
...
```bash
bash scripts/ai/sync_skill_lib.sh --check
```
