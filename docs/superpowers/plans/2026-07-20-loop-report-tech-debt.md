# loop-report-tech-debt Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Introduce the `loop-report-tech-debt` report loop (L2 from day one) with design documentation first, then rename package/skill, enrich detect JSON, and add the caller workflow.

**Architecture:** Report loops use the `loop-report-<domain>` naming family. Detect scripts emit all skill-input fields (same layer as `detect_changes.sh` / `detect_changelog_commits.sh`). L2 writes `docs/report/report-tech-debt/YYYY-MM-DD.md` via merge-gated PR. No GitHub Issue creation at any level.

**Tech Stack:** APM packages, GitHub Actions (`ci-loop-caller.yaml`), bash detect scripts, loop-report-tech-debt skill references.

## Global Constraints

- Report family prefix: `loop-report-<domain>` (e.g. `loop-report-tech-debt`, future `loop-report-errors`).
- Full APM rename: `.apm/packages/loop-tech-debt` → `loop-report-tech-debt` (package, skill, detect, evals).
- `loop_name`: `report-tech-debt`; state: `.loop/state-report-tech-debt.json`.
- Caller workflow: `on-loop-report-tech-debt.yaml`.
- Report path: `docs/report/report-tech-debt/YYYY-MM-DD.md`.
...
- Detect enrich fields (`report_file`, `previous_report`) live in `detect_report_tech_debt.sh`; sensors live in `detect_report_tech_debt_sensors.sh`.
...
- Create: `docs/explanation/loop-engineering/workflows/loop-report-tech-debt-workflow-design.md`
...
Create `loop-report-tech-debt-workflow-design.md` following `loop-docs-triage-workflow-design.md` structure. Include:
...
| ------------------------------- | ----------------------------------------------------------------------------------------------- |
| `level`                         | `L2`                                                                                            |
| `loop_name`                     | `report-tech-debt`                                                                              |
| `skill_name`                    | `loop-report-tech-debt`                                                                         |
| `detect_script`                 | `.agents/skills/loop-report-tech-debt/scripts/detect_report_tech_debt.sh`                       |
...
| `engine`                        | `cursor`                                                                                        |
| `pr_title`                      | `docs(report): technical debt report (loop-report-tech-debt)`                                   |
...
- `loop-caller-inputs-reference.md` (design doc link + caller `on-loop-report-tech-debt.yaml`)
- `multi-branch-loops-design.md` workflow design documents table
- `index.md` topic index
- `mkdocs.yml` nav: `- Report Tech Debt: explanation/loop-engineering/workflows/loop-report-tech-debt-workflow-design.md`

- [ ] **Step 3: Validate markdown**

Run: `bash .agents/skills/markdown-validation/scripts/validate.sh docs/explanation/loop-engineering/workflows/loop-report-tech-debt-workflow-design.md docs/explanation/loop-engineering/index.md mkdocs.yml`
...
- Rename: `.apm/packages/loop-tech-debt/` → `.apm/packages/loop-report-tech-debt/`
- Update: `apm.yml` dependency entry
- Regenerate: `apm install --update`

Rename skill references: `loop-tech-debt` → `loop-report-tech-debt` throughout package.
...
- Rename: `detect_tech_debt.sh` → `detect_report_tech_debt.sh`
...
`report_file` = `docs/report/report-tech-debt/$(date -u +%Y-%m-%d).md`
...
- Create: `.github/workflows/on-loop-report-tech-debt.yaml`
- Create: `.github/workflows/example/on-loop-report-tech-debt.yaml` (remote SHA pin)
- Update: `.loop/loop-budget.json` with `report-tech-debt` entry
