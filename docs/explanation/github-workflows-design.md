...
**Exception — loop callers:** `on-loop-*.yaml` pass configuration via `with:` on `ci-loop-caller.yaml` (no caller `env:`), matching `on-ci-push-*.yaml`. See [Loop Caller Reusable Workflow Design](loop-engineering/loop-caller-reusable-design.md). Legacy caller `env:` names: [Loop Caller `env` Reference](loop-engineering/workflows/loop-caller-env-reference.md).
