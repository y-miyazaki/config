...
Workflow and domain design for the `loop-report-tech-debt` (`report-tech-debt`) loop.
...
**Artifacts:** `on-loop-report-tech-debt.yaml` · skill `loop-report-tech-debt` · `scripts/detect_report_tech_debt.sh`

Shared caller keys: [Loop Caller Inputs Reference](loop-caller-inputs-reference.md).

## Purpose

Run a full-repository mechanical technical-debt scan, classify findings via the skill taxonomy, and open an L2 merge-gated PR that writes a dated report under `docs/report/report-tech-debt/`.
...
- Write `docs/report/report-tech-debt/YYYY-MM-DD.md` via L2 review PR
...
Skill execution boundaries: `loop-report-tech-debt` SKILL.md (`USE FOR` / `DO NOT USE FOR`).
...
Keys are passed in `on-loop-report-tech-debt.yaml` via `with:` on `ci-loop-caller.yaml` (alphabetically ordered). Multiline values (`agent_verifier_criteria`, `pr_body`, `prompt_instructions`) are defined inline in the caller workflow.
...
| `allowlist`                    | Comma-separated globs the implementer may modify. Must align with report path scope.                                                                                                   | `docs/report/report-tech-debt/**/*.md`                                                          |
...
| `detect_domain_env_json`       | JSON object forwarded to detect script env. Optional `REPORT_TECH_DEBT_*` keys override report paths; defaults match dogfood layout.                                                   | `'{}'`                                                                                          |
| `detect_script`                | Domain detect script path.                                                                                                                                                             | `.agents/skills/loop-report-tech-debt/scripts/detect_report_tech_debt.sh`                       |
...
| `loop_name`                    | Loop identifier; state file `.loop/state-report-tech-debt.json`.                                                                                                                       | `report-tech-debt`                                                                              |
...
| `pr_enabled` / `pull_requests` | Enumerate open PR heads. Report-tech-debt uses integration branches only.                                                                                                              | `false`                                                                                         |
| `pr_title`                     | PR title when finalize strategy is `open_pr`.                                                                                                                                          | `docs(report): technical debt report (loop-report-tech-debt)`                                   |
...
| `skill_name`                   | Skill package to invoke.                                                                                                                                                               | `loop-report-tech-debt`                                                                         |

## Detect

### Integration mode only

Per watch branch, `loop-detect` checks out the branch and invokes `detect_report_tech_debt.sh` with `targets["integration:<branch>"].last_sha`.
...
| `report_file`     | Target path: `<REPORT_TECH_DEBT_DIR>/<UTC-date>.md` (defaults under `docs/report/report-tech-debt/`)                       |
| `previous_report` | Latest dated report under `REPORT_TECH_DEBT_DIR` plus optional `REPORT_TECH_DEBT_LEGACY_SEARCH_DIRS`; empty string if none |

**Default scope:** full repository (sensors read source for evidence; `docs/report/**` excluded from sensors per skill references).

**Skill** (`loop-report-tech-debt`) classifies signals into prioritized findings and writes `report_file` at L2.
...
- **APPROVE** when diff touches only `docs/report/report-tech-debt/**/*.md`, report content matches detect signals, paths cited in report exist in the repository, Critical+High count ≤ 25, and no denylist paths modified
