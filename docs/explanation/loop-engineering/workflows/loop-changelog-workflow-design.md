...
Keys are passed in `on-loop-changelog.yaml` via `with:` on `ci-loop-caller.yaml` (alphabetically ordered). Multiline values (`agent_verifier_instructions`, `agent_implementer_instructions`) are defined inline in the caller workflow.
...
| `agent_verifier_instructions`                            | Verifier rubric: Unreleased mapping, detect `releases[]` promotion, no hallucinated versions.                                                                                                                             | Inline in caller workflow                                      |
