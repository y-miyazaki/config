...
| `LOOP_FINALIZE_INTEGRATION`      | Finalize for integration targets. Wire default `open_pr`. Exception: `push`.                                                                                                           | `open_pr`                          |
| `LOOP_FINALIZE_PULL_REQUEST`     | Finalize for PR-head targets. Wire default `push_head`. Migration to `open_pr` pending.                                                                                                | `push_head` (ci-sweeper)           |
...
| `LOOP_PR_REQUIRE`                | PR require tokens (all must match). Empty when `LOOP_PULL_REQUESTS=true` → no PR-head targets (fail-closed).                                                                           | `label:ci-sweeper-ok` (ci-sweeper) |
