...
3. Field semantics and `## Constraints` edit gate: [common-loop-triage-format.md](../common-loop-triage-format.md) and skill `references/category-input-schema.md` / `category-automation-envelope.md` (automation path only).
...
Keys are passed in `on-loop-docs-updater.yaml` via `with:` on `ci-loop-caller.yaml` (alphabetically ordered). Multiline values (`agent_verifier_instructions`, `agent_implementer_instructions`) are defined inline in the caller workflow.
...
| `agent_verifier_instructions`                             | Verifier APPROVE/REJECT rubric. Doc-only edits; factual consistency; no sensitive data.                                                                                                                                   | Inline in caller workflow                                                 |
