...
- `.apm/packages/loop/`: generic loop checker skill (`loop-verifier`); callers pass `agent_checker_skill_name` to `loop-execute` — domain entry skills are not here
