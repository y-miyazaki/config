...
- [Runtime Guardrails: NeMo Guardrails vs Guardrails AI vs Llama Guard](#runtime-guardrails-nemo-guardrails-vs-guardrails-ai-vs-llama-guard)
  - [Guidelines](#guidelines-1)
- [Agent Skills Lint / Evaluation: Waza vs markdownlint-cli2 vs apm audit](#agent-skills-lint--evaluation-waza-vs-markdownlint-cli2-vs-apm-audit)
  - [Guidelines](#guidelines-2)
- [Agent Observability \& Audit: LangSmith vs Arize Phoenix vs OpenLLMetry](#agent-observability--audit-langsmith-vs-arize-phoenix-vs-openllmetry)
...
## Runtime Guardrails: NeMo Guardrails vs Guardrails AI vs Llama Guard

| 比較項目                 | NeMo Guardrails                                           | Guardrails AI                                              | Llama Guard                                                  |
| ------------------------ | --------------------------------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------ |
| 提供元                   | NVIDIA                                                    | Guardrails AI                                              | Meta                                                         |
| リポジトリ               | [NVIDIA/NeMo-Guardrails](https://github.com/NVIDIA/NeMo-Guardrails) | [guardrails-ai/guardrails](https://github.com/guardrails-ai/guardrails) | [meta-llama/PurpleLlama](https://github.com/meta-llama/PurpleLlama) |
| ライセンス               | Apache-2.0                                                | Apache-2.0                                                 | Llama Community License (要確認)                             |
| 提供形態                 | OSS フレームワーク                                        | OSS ライブラリ/フレームワーク                              | OSS セーフティモデル                                          |
| 主用途                   | 対話フロー制御 / 実行時ガードレール                       | 入出力バリデーション / ルール適用                          | 有害・不適切コンテンツ判定                                    |
| プロンプトインジェクション対策 | ✅ (フロー制御で抑制)                                     | ⚠️ (ルール設計依存)                                         | ⚠️ (分類モデル補助として利用)                                 |
| PII / 機密情報フィルタ   | ⚠️ (実装次第)                                              | ✅ (バリデータ拡張)                                         | ⚠️ (用途に応じた別実装が必要)                                 |
| エージェント統合         | ✅ (ワークフロー組込み)                                     | ✅ (アプリ層に組込み)                                       | ⚠️ (推論パイプラインへの組込みが必要)                         |
| 監査向けログ親和性       | 中                                                         | 中                                                          | 中                                                            |
...
## Agent Skills Lint / Evaluation: Waza vs markdownlint-cli2 vs apm audit

| 比較項目                 | Waza                                              | markdownlint-cli2                           | apm audit                                      |
| ------------------------ | ------------------------------------------------- | ------------------------------------------- | ---------------------------------------------- |
| 提供元                   | Microsoft                                          | David Anson                                  | APM                                             |
| リポジトリ               | [microsoft/waza](https://github.com/microsoft/waza) | [DavidAnson/markdownlint-cli2](https://github.com/DavidAnson/markdownlint-cli2) | - (local package) |
| ライセンス               | MIT                                                | MIT                                          | Apache-2.0 (要確認)                             |
| 主用途                   | Agent Skills の readiness/eval                    | Markdown の静的 lint                         | APM 依存関係・構成の監査                         |
...
| 導入効果                 | Agent Skills の品質保証を直接評価                  | 文書品質を安定化                              | 構成と依存の監査一貫性を担保                      |
...
## Agent Observability & Audit: LangSmith vs Arize Phoenix vs OpenLLMetry

| 比較項目                 | LangSmith                                                | Arize Phoenix                                             | OpenLLMetry (Traceloop)                                 |
| ------------------------ | -------------------------------------------------------- | --------------------------------------------------------- | -------------------------------------------------------- |
| 提供元                   | LangChain                                                | Arize                                                     | Traceloop                                                 |
| リポジトリ               | - (商用 SaaS)                                            | - (OSS / Cloud)                                          | - (OSS) |
| ライセンス               | 商用                                                    | Elastic License 2.0                                       | Apache-2.0                                                |
| 提供形態                 | SaaS 中心                                                | OSS + Cloud                                               | OSS (OpenTelemetry ベース)                               |
| 主用途                   | Agent 実行トレース / 評価 / デバッグ                     | LLM/Agent 観測・評価                                      | LLM 呼び出しとツール実行の標準トレース収集               |
| ツール呼び出し監査       | ✅                                                        | ✅                                                         | ✅                                                        |
...
| ------------------------ | ------------------------------------------------------- | --------------------------------------------------------- | -------------------------------------------------------- |
| 提供元                   | APM                                                     | GitHub/Git                                                   | ベンダー各社                                               |
| リポジトリ               | - (local package)                                        | - (Git 管理)                                             | - (vendor provided)                                   |
| ライセンス               | Apache-2.0 (要確認)                                     | リポジトリ定義に依存                                       | ベンダー/プラットフォーム規約に依存                       |
