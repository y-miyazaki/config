...
# - detect_changes range scope detects .github workflow changes
...
# - detect_changes without DOCS_TRIAGE_DOC_GLOBS discovers scannable markdown excluding agents
...
@test "detect_changes range scope detects github workflow changes" {
    git_test_repo_setup
...
@test "detect_changes range scope asserts exact changed_files for github workflow edits" {
    ...
    run jq -e '
        ...
        and (.changed_files | index(".github/workflows/ci.yaml")) != null
    ' <<< "${output}"
    [ "$status" -eq 0 ]
}
...
@test "detect_changes range scope includes affected docs when markdown is renamed" {
    ...
    run jq -e '
        ...
        and (.affected_docs | index("docs/new.md")) != null
    ' <<< "${output}"
    [ "$status" -eq 0 ]
}
...
@test "detect_changes without DOCS_TRIAGE_DOC_GLOBS discovers scannable markdown excluding agents" {
    git_test_repo_setup
