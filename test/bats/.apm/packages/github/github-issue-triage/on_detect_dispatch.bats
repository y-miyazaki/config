...
HOOK_SCRIPT="$(apm_skill_script_path github-issue-triage hooks/on_detect_dispatch.sh)"
...
@test "on_detect_dispatch dry-run logs event type when dispatch_requested true" {
    printf '%s' '{"status":"ok","skip":false,"result":{"dispatch_requested":true,"dispatch_event_type":"loop-github-issue-autofix","dispatch_client_payload":{"issue_number":"12"}}}' \
        > "${BATS_TEST_TMPDIR}/d.json"
    run env DISPATCH_DRY_RUN=1 bash "${HOOK_SCRIPT}" "${BATS_TEST_TMPDIR}/d.json"
    [ "$status" -eq 0 ]
    [[ ${output} == *"loop-github-issue-autofix"* ]] || [[ ${stderr} == *"loop-github-issue-autofix"* ]]
}
...
@test "on_detect_dispatch rejects invalid dispatch_client_payload" {
    printf '%s' '{"status":"ok","skip":false,"result":{"dispatch_requested":true,"dispatch_event_type":"loop-github-issue-autofix","dispatch_client_payload":{"issue_number":"0","extra":"x"}}}' > "${BATS_TEST_TMPDIR}/d.json"
    run bash "${HOOK_SCRIPT}" "${BATS_TEST_TMPDIR}/d.json"
    [ "$status" -ne 0 ]
}
