...
# Tests for .apm/packages/repo-maintenance/.apm/skills/tech-debt/scripts/detect_tech_debt.sh
#
# Use cases:
...
# - detect_tech_debt emits report_file and previous_report enrich fields
# - detect_tech_debt previous_report excludes today's dated report file
...
@test "detect_tech_debt excludes marker signals under report parent directory" {
    git_test_repo_setup
    mkdir -p "${GIT_TEST_REPO}/docs/report/tech-debt" "${GIT_TEST_REPO}/src"
    printf 'package main\n// TODO: report noise\nfunc main() {}\n' \
    ...
    [[ $output == *'"todo_comment"'* ]]
    [[ $output == *'src/app.go'* ]]
    [[ $output != *'docs/report/tech-debt/noise.go'* ]]
}
...
@test "detect_tech_debt honors TECH_DEBT_DIR for report_file path" {
    git_test_repo_setup
    local today_date
...
@test "detect_tech_debt previous_report excludes today's dated report file" {
    git_test_repo_setup
    local today_date older_report
    today_date="$(date -u +%Y-%m-%d)"
    older_report="docs/report/tech-debt/2020-01-01.md"
    ...
    [[ $output == *"\"previous_report\": \"${older_report}\""* ]] \
        || [[ $output == *"\"previous_report\":\"${older_report}\""* ]]
    [[ $output != *"\"previous_report\": \"docs/report/tech-debt/${today_date}.md\""* ]]
    [[ $output != *"\"previous_report\":\"docs/report/tech-debt/${today_date}.md\""* ]]
}

@test "detect_tech_debt previous_report falls back to legacy tech-debt directory" {
    git_test_repo_setup
    ...
    [[ $output == *"\"previous_report\": \"${legacy_report}\""* ]] \
        || [[ $output == *"\"previous_report\":\"${legacy_report}\""* ]]
    [[ $output == *"\"report_file\": \"docs/report/tech-debt/${today_date}.md\""* ]] \
        || [[ $output == *"\"report_file\":\"docs/report/tech-debt/${today_date}.md\""* ]]
}

@test "detect_tech_debt previous_report picks latest dated md when older reports exist" {
    git_test_repo_setup
    mkdir -p "${GIT_TEST_REPO}/docs/report/tech-debt"
...
@test "detect_tech_debt report_file matches UTC date pattern" {
    ...
    [ "$status" -eq 0 ]
    assert_detect_tech_debt_ok_json "${output}" "all" ""
    [[ $output == *"\"report_file\": \"docs/report/tech-debt/${expected_date}.md\""* ]] \
        || [[ $output == *"\"report_file\":\"docs/report/tech-debt/${expected_date}.md\""* ]]
}
...
@test "detect_tech_debt truncates dependency signals when dep cap is reached" {
    git_test_repo_setup
    local dep_idx
    {
        printf '{\n  "name": "dep-cap-test",\n  "dependencies": {\n'
