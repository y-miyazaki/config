...
# Tests for .apm/packages/repo-maintenance/.apm/skills/changelog/scripts/detect_changelog_commits.sh
...
DETECT_SCRIPT="$(apm_skill_script_path changelog detect_changelog_commits.sh)"

setup() {
    bats_source_apm_skill changelog detect_changelog_commits.sh
}
...
@test "is_loop_maintenance_commit skips chore changelog scope" {
    run is_loop_maintenance_commit "chore" "changelog" "update CHANGELOG.md (changelog)"
    [ "$status" -eq 0 ]
}
...
@test "detect_changelog_commits skips loop maintenance commit in range" {
    ...
    local base
    base="$(git -C "${GIT_TEST_REPO}" rev-parse HEAD)"
    git_test_repo_commit "chore(changelog): update CHANGELOG.md (changelog)"
    git_test_repo_run "bash '${DETECT_SCRIPT}' --scope range --since '${base}'"
    [ "$status" -eq 0 ]
    assert_detect_changelog_ok_json "${output}" "range" "${base}"
