...
| ------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------- |
| `.apm/packages/<name>/`                                            | `.agents/`, `.claude/`, `.codex/`, `.cursor/`, `.kiro/`, `.vscode/`                                  |
| `scripts/lib/` (then sync to skills)                               | `.apm/packages/*/skills/*/scripts/lib/` directly                                                     |
| `scripts/{shell-script,go,terraform}/` ↔ matching skill `scripts/` | See [Validation Scripts Mirror](#validation-scripts-mirror-scripts--skill) — keep both sides in sync |
...
Domain validation entrypoints exist in **two places** in this repository. Keep them aligned in the same change:

| Domain         | Skill source (`.apm/packages/`)                                           | Repository scripts (`scripts/`) |
| -------------- | ------------------------------------------------------------------------- | ------------------------------- |
| `shell-script` | `.apm/packages/shell-script/.apm/skills/shell-script-validation/scripts/` | `scripts/shell-script/`         |
| `go`           | `.apm/packages/go/.apm/skills/go-validation/scripts/`                     | `scripts/go/`                   |
| `terraform`    | `.apm/packages/terraform/.apm/skills/terraform-validation/scripts/`       | `scripts/terraform/`            |

**Bidirectional rule**: editing either side requires updating the other in the same PR. Skill and `scripts/` copies must stay behavior-identical aside from path layout below.

**Path layout (do not unify literally)**:
...
`WORKSPACE_ROOT` differs only for `shell-script` (skill tree is deeper). `go` and `terraform` differ only in the library import lines.

**Workflow**:

1. Implement the change on whichever side you are editing.
2. Copy to the paired path and adjust only the rows in the table above.
3. Run the relevant Bats suites under `test/bats/scripts/<domain>/` when behavior changes.

`scripts/lib/` still follows the one-way sync in the previous bullet (`sync_skill_lib.sh` → skill `scripts/lib/`). Do not edit skill `scripts/lib/` directly.
