#!/bin/bash
#######################################
# Description: Post or correct issue-triage GitHub Issue comments
#
...
# Global variables
#######################################
# COMMENT_MARKER: HTML marker that identifies issue-triage bot comments (do not PATCH unmarked comments)
COMMENT_MARKER="<!-- issue-triage:v1 -->"
...
Description: Post a new issue-triage comment, or correct the latest marked bot comment
...
function correct_comment {
    ...
    if [[ -z ${comment_id} || ${comment_id} == "null" ]]; then
        error_exit "No issue-triage marker comment to correct on #${ISSUE_NUMBER}"
    fi
    body="$(cat "${body_file}")"
